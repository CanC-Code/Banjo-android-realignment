...
void osViSetEvent(void *mq, void *m, u32 count)      { (void)mq; (void)m; (void)count; }
void osCreateViManager(void *pri)                    { (void)pri; }
...
// NOTE: gFramebuffers, g_active_fb_offset, gFramebufferWidth, gFramebufferHeight
// are now defined in lowlevel_bridge.cpp to ensure C++ linkage visibility.
